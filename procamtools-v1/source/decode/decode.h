...
private:
	...
	void decode_gray(const std::vector<slib::Field<2,float>>& images, int direction)
	{
		...
		{
			float maxval = std::max(images[2*bit].max(), images[2*bit+1].max()); //Saca el brillo máximo de cada imagen y su complementaria
			diff[nbits-1-bit] = images[2*bit]-images[2*bit+1];  //Saca la diferencia entre las imagenes para calcular el error de gris.
			...
			CountGraycodeUncertainty(diff[nbits-1-bit], threshold, m_gray_error[direction]); //Mascarea los pixeles que tienen valor definido blanco o negro
		}
